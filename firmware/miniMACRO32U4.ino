...
SimpleRotary rotaryA(0,1,4); // Pin A, Pin B, Button Pin
SimpleRotary rotaryB(2,3,8); // Pin A, Pin B, Button Pin
SimpleRotary rotaryC(7,9,6); // Pin A, Pin B, Button Pin

byte buttonPin[]={21, 14, 15, 10, 16};
...
void kPress (byte encoder, bool cw){ //encoder and clockwise?
  for (int i=0; i<3; i++) Keyboard.press(rotVal[encoder][cw][i]);
  Keyboard.releaseAll();
}
